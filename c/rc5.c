...
fprintf(stderr, "A = %u, B = %u\n", A, B);
fprintf(stderr, "i=%u, ky[i]=%u\n",i,ky[i]);
fprintf(stderr, "ky[i]+A+B = %u\n", ky[i] + A + B);
#endif

                A = *(ky+i) = ROTL((*(ky+i) + A + B), 3&ANDVAL);
#ifdef debug_key_sched
fprintf(stderr, "A --> %u\n",A);
fprintf(stderr, "ky[%u] --> %u\n", i, ky[i]);
#endif

#ifdef debug_key_sched
fprintf(stderr,"------------------------------calc B\n");
fprintf(stderr, "A = %u, B = %u\n", A, B);
fprintf(stderr,"j=%u, L[j]=%u\n",j, L[j]);
fprintf(stderr,"L[j]+A+B = %u\n",*(L+j)+A+B);
